...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(resources))
        for resource in resources:
            group_id = get_group_id(token, version, resource.link)
            if group_id is not None:
                group_posts = get_vk_wall_posts(token, version, group_id, end_date, resource.name, resource)
                

        self.stdout.write(self.style.SUCCESS('Successfully parsed and saved VK data'))